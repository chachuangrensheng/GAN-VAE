...
class VAEGAN(object):
    def __init__(self, sess, flags, input_height=88, input_width=int(barTime * FS), batch_size=64, sample_num=3,
    ...
    def load_images(self):
        ...
        # 设置 NumPy 随机模块的种子
        np.random.seed(seed)
        # 使用 TensorFlow 操作生成随机索引
        indices = tf.range(num_images)
        shuffled_indices = tf.random.shuffle(indices, seed=seed)  # 使用相同的种子
        # 使用随机索引来打乱X和xp
        X = tf.gather(X, shuffled_indices)
        xp = tf.gather(xp, shuffled_indices)
        # 转换为numpy数组
    ...
    def load_image(self):
        ...
        # 返回图像批次和对应的标签列表
        return images_batch, tf.convert_to_tensor(self.labels)
    ...
    def train(self, config):
        ...
        for epoch in xrange(config.epoch):
            ...
            if features_list.ndim == 3:
                features_list = features_list.reshape(features_list.shape[0], -1)
            fz = pca.fit_transform(features_list)

            # Calculate the anomaly score
            normal_scores = fz  # Assuming the first half are normal data
            ...
            # del self.data_test, self.data_y
            # gc.collect()  # 强制进行垃圾回收
    ...
    def extract_features(self, data):
        ...
        if self.Flags.train:
            return self.sess.run(self.feature_op, feed_dict={self.x0: data_np})
        else:
            return self.sess.run(self.feature_op, feed_dict={self.x0test: data_np})
