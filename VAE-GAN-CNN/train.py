...
if __name__=='__main__':
    ...
    #     show_and_save(f'image/train_noise_epoch_{epoch}', make_grid((c * 0.5 + 0.5).cpu(), 8))
    #     show_and_save(f'image/train_epoch_{epoch}', make_grid((b*0.5+0.5).cpu(),8))
    #
    # # 训练结束后保存最后的模型状态
    ...
    #
    # # 绘制损失曲线
    # plot_loss(prior_loss_list, 'prior_loss0.png')  # 先验损失误差
    # plot_loss(recon_loss_list, 'recon_loss0.png')  # 重建损失误差
    # plot_loss(gan_loss_list, 'gan_loss0.png')   # GAN损失误差
    #
